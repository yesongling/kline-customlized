...
export class Chart {
    ...
    updateDataAndDisplay() {
        ...
        Kline.instance.polymerization = this._range;
        ChartManager.instance.setCurrentDataSource('frame0.k0', this._symbol + '.' + this._range);  // cache storage
        ChartManager.instance.setNormalMode();
        let f = Kline.instance.chartMgr.getDataSource("frame0.k0").getLastDate();  // last data record timestamp
    ...
    setCurrentPeriod(period) {
        ...
        Kline.instance.polymerization = period.replace(/m$/,'m').replace(/^0+/,'').replace(/d$/,'day').replace(/h$/,'hour').replace(/w$/,'week');
